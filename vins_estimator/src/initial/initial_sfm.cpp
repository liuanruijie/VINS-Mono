			  ...
			  vector<SFMFeature> &sfm_f, map<int, Vector3d> &sfm_tracked_points)
{
	...
	Eigen::Matrix<double, 3, 4> Pose[frame_num]; // 滑动窗口中各帧在世界坐标系（个人推断，一开始是把第l帧相机坐标系作为世界坐标系）中的位姿

	// 第l帧
	c_Quat[l] = q[l].inverse(); // 四元数取逆（实际上就是共轭），相当于旋转矩阵取逆
	c_Rotation[l] = c_Quat[l].toRotationMatrix(); // 四元数转旋转矩阵
	c_Translation[l] = -1 * (c_Rotation[l] * T[l]); // 位移
